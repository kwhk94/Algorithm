...
namespace AlgorithmMain
{
    public class BFS_Test
    {
        ...
        {
            BFS<int> objectBFS = new BFS<int>(nodeList);
            Console.WriteLine("========== 시작노드부터 마지막 노드까지 수평 순서로 Draw =====");
            objectBFS.SimpleBFS_showAllNode(0);
        }
        ...
        public void SearchNode(int _startIndex, int _targetIndex)
        {
            ...
            BFS<int> objectBFS = new BFS<int>(nodeList);
            Console.WriteLine("========== 시작노드부터 찾으려는 대상 노드까지 수평 순서로 Draw =====");
            objectBFS.BFS_PathSearch(_startIndex, _targetIndex);
        }
    }
}
